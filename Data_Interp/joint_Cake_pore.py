...
def plot_pore_violins_by_code(
    ...
    *,
    ...
    """
    Plot final moisture (Cake_por) as violin plots for all Sample Codes
    ...
    if "Sample Code" not in df.columns or "Cake_por" not in df.columns:
        raise ValueError("DB sheet must contain 'Sample Code' and 'Cake_por' columns.")

    df["Cake_por"] = pd.to_numeric(df["Cake_por"], errors="coerce")
    ...
    df = df.dropna(subset=["Sample Code", "Cake_por"])

    # Convert to % if stored as 0–1
    #if df["Cake_por"].max() <= 1.5:
     #   df["Cake_por"] = df["Cake_por"] * 100.0

    # --- Group for violins ---
    by_code = (
        df.groupby("Sample Code")["Cake_por"]
    ...
    if show_points:
        ...
        for i, code in enumerate(codes):
            ...
            for (xpos, y, tp_label) in zip(xj, sub["Cake_por"].to_numpy(), sub["TP_norm"].to_numpy()):
                ax.scatter(
                    xpos, y,
                    s=32,
                    edgecolor="black",
    ...
    ax.set_ylabel("Cake Porosity")
    ax.set_title("Cake Porosity by Sample Code")
